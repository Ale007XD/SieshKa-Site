...
def get_menu_cache_key(
    day: str, method: str, slot: Optional[str], tz: str, version: int
) -> str:
...
def convert_to_core_rule(rule: AvailabilityRule) -> Optional[CoreRule]:
    ...
    try:
        ...
        methods = (
            rule.methods
            if rule.methods and len(rule.methods) > 0
        ...
        return CoreRule(
            ...
            if rule.allow_tomorrow is not None
            else True,
...
async def get_available_slots(
    ...
    # Generate cache key
    cache_key = get_slots_cache_key(
        day, method, config.business_tz, config.menu_version
    )
...
async def get_menu(
    ...
    # Generate cache key
    cache_key = get_menu_cache_key(
        day, method, slot, config.business_tz, config.menu_version
    )
    ...
    try:
        ...
        with SessionLocal() as db:
            ...
            all_rules_db = (
                db.query(AvailabilityRule)
                .filter(AvailabilityRule.is_active == True)
                .all()
            )
            all_core_rules = [
                r
                for r in (convert_to_core_rule(rule) for rule in all_rules_db)
                if r is not None
            ]
                        ...
                        try:
                            ...
                            cat_products.append(
                                MenuItemAvailability(
                                    ...
                                    next_available=format_next_available(
                            ...
                            logger.error(
                                f"Error checking availability for product {product.id} ({product.name}): {e}"
                            )
...
async def refresh_menu_cache():
    ...
    with SessionLocal() as db:
        ...
        if config:
            ...
            db.commit()
            return {
                "message": "Menu cache refreshed",
                "new_version": config.menu_version,
            }
        else:
