...
def _add_to_dlq(chat_id: str, text: str):
    ...
    failed_notifications.append(
        {"chat_id": chat_id, "text": text, "timestamp": datetime.now().isoformat()}
    )
    ...
    logger.warning(
        f"Added notification to DLQ. Queue size: {len(failed_notifications)}"
    )
...
async def retry_failed_notifications() -> int:
    ...
    for notification in failed_notifications:
        result = await send_message(
            notification["chat_id"], notification["text"], max_retries=1
        )
        if result:
            successful += 1
        else:
    ...
    logger.info(
        f"Retry complete. Successful: {successful}, Still failed: {len(still_failed)}"
    )
    return successful
